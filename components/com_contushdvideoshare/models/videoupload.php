		...
		if ($uploadbutton)
		{
			...
			if ($videourl == '1' || $normalvideoformval)
			{
				...
				elseif (strlen(JRequest::getVar('ffmpeg', '', 'post', 'string')) > 0)
				{
					...
					$ffmpegVal = strrev(JRequest::getVar('ffmpeg', '', 'post', 'string'));
					...
					exec($strFfmpegPath . ' ' . '-i' . ' ' . $destFile . ' ' . '-sameq' . ' ' . $target_path2 . '  2>&1');
					exec($strFfmpegPath . " -i " . $target_path2 . ' ' . "-an -ss 00:00:03 -an -r 1 -s 120x68 -f image2"
							. ' ' . $vpath . $filename[0] . "_thumb.jpg"
									);
					exec(
					$strFfmpegPath . " -i " . $target_path2 . ' ' . "-an -ss 00:00:03 -an -r 1 -vframes 1 -y"
							. ' ' .$vpath . $filename[0] . '_preview' . ".jpg"
									);
					$url = $filename[0] . '.flv';
					// Getting Hd path
					$hd = '';
			...
			if ($videotype == 'edit')
			{
				...
				if ($seltype == 0 || $seltype == 2 || $seltype == 1 || $seltype == 4 || $seltype == 6)
				{
