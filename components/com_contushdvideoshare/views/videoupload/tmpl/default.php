...
<div class="player clearfix" id="clsdetail">
		...
		<div class="floatleft allform_left">
			...
			<ul id="upload_thread">
				...
				if (isset($upload_methods['FFMPEG']))
				{
					?>
									<li>
										<input type="radio"  class="butnmargin" name="filetype" id="filetype6" value="5"
											   onclick="filetypeshow(this);"/> <span class="select_upload"><?php echo JText::_('HDVS_FFMPEG'); ?></span>
									</li>
								<?php
								}
				?>
	...
	<div id="typeff" name="typeff">
		<div class="allform" >
			<table  class="table_upload" cellpadding="0" cellspacing="0" width="100%">
					...
					<td>
						<div id="f11-upload-form" >
							<form name="ffmpeg" method="post" enctype="multipart/form-data" >
								...
								<input  type="button" name="uploadBtn" value="<?php echo JText::_('HDVS_UPLOAD_VIDEO'); ?>"
										disabled="disabled" class="button cursor_pointer upload_video" onclick="return addQueue(this.form.name, this.form.myfile.value);" />
								<label id="lbl_normal">
									<?php
									if (isset($videoedit->filepath))
									{
										if ($videoedit->filepath == 'FFmpeg')
										{
...
?>
		...
		<input type="hidden" name="ffmpeg" id="ffmpeg" value="<?php
		if (isset($videoedit->filepath))
		{
			if ($videoedit->filepath == 'FFmpeg')
			{
...
elseif (isset($upload_methods['FFMPEG']))
{
	?>
...
elseif (isset($videoedit->filepath) && $videoedit->filepath == 'FFmpeg')
{
	?>
		filetypeshow("5");
		document.getElementById("filetype6").checked = true;
